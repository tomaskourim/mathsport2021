...
examined in and its theoretical background was described in more detail.
...
the random walk was described in much detail and many alternation
...
can define 2 major types of random walk with varying probability -\emph{
...
being$1$ again decreases by a parameter $\lambda$ so that $p_{t+1}=\lambda p_{t}$.
...
As mentioned in Section\ref{sec:Random-walk-with}, there are 4 basic
...
does not provide reasonable results. In fact, bookmakers margin is
...
$n_{1}$ and $n_{2}$ such that $\mu(o,0)=n_{2}(o)$ and $\mu(o,2)=n_{1}(o)$.

To obtain reasonable estimation of the model parameter $p_{0}$ from
provided odds, the first step is to estimate parameter $t$. This
...
of coefficient $\lambda$ was found again using the maximum likelihood
estimate
\[
...
A won the set, $x_{ij}=0$ and $p_{ij}$ is the probability of Player
A winning the set computed using the currently considered model variant.

Finally, the Akaike Information Criterion $AIC=2k-2ln(L)$, which
considers the number of parameters to correctly the best model, was
used. Here $k$ is the number of model parameters and $L$ is the
maximal likelihood.
...
For each match in the dataset, first the starting probability was
derived using the procedure described in Section \ref{subsec:Starting-probability-selection}.
Then, it was iterated over the sets played, and for each set, the
...
\ref{subsec:Model-selection}. This probability was compared to odds
provided by the bookmaker and possibly a virtual bet was made.
...
Then, the probability base strategy, where $p$ (probability of winning)
units was bet. Finally the odds based strategy, where $\frac{1}{odds}$
...
i.e. in case of fair odds \cite{ja2015ddny}. Besides the different
...
i.e. $\frac{total\,profit}{total\,bankroll\,needed}.$ For this parameter,
the naive betting yields $ROI=148\%$, probability based betting $ROI=98\%$
...
$a$ is the odds provided by bookmaker.}
\end{table}

\begin{figure}
\includegraphics[width=1\textwidth]{account_balance_development}\caption{\label{fig:A-graph-showing}A graph showing the account balance development
